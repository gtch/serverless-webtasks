...
const infoService = {
  ...
  gatherServiceInfo() {
    ...
    return this.getProfileOption().then(profile => {
      profile = profile || 'serverless';
      return this.getStageOption().then(stage => {
        this.webtaskInfo = this.webtaskInfo || { service, stage, profile };
  ...
  displayServiceInfo() {
    ...
    message += `${chalk.yellow('profile:')} ${info.profile}\n`;
  ...
  displayEndpoints() {
    ...
    if (webtasks && webtasks.length > 0) {
      _.each(webtasks, webtask => {
        endpointsMessage += `\n  * - ${webtask.url}`;
        if (webtask.host) {
          endpointsMessage += `\n    - https://${webtask.host}/${webtask.container}/${webtask.deployedName}`;
        }
      });
    } else {
      endpointsMessage += '\n  None';
