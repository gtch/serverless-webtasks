...
function mapWebtasks(webtask, error) {
  return {
    ...
    host: webtask ? webtask.claims.host : null,
...
const profileService = {
  ...
  getProfile() {
    return this.getProfileOption().then(profileOption => {
      if (profileOption) {
        return this.getProfileByName(profileOption);
      }
        ...
        return this.getProfile().then(profile => {
          return this.getStageOption().then(stage => {
            ...
            if (!functionImport || !isValid) {
              ...
              cli.consoleLog(`- Export a default function ('module.exports =') from file ${pathName}`);
              cli.consoleLog(separator);
              process.exit(1);
            }
